...
type CustomLineItem = NonNullable<NonNullable<Parameters<typeof checkout.createCheckout>[0]>['customLineItems']>[0];
...
export async function getCheckoutUrlForCustomLineItem(customLineItem: CustomLineItem) {
  const checkoutResult = await checkout.createCheckout({
    customLineItems: [
      {
        ...customLineItem,
