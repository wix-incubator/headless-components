...
export default function PostPaywall({
  ...
  const subscribeNowCta = (
    <Dialog>
      ...
      <DialogContent className="max-h-[100dvh] max-w-screen-xl overflow-auto sm:max-h-[95dvh] sm:w-[calc(100vw-32px)] sm:px-6">
        ...
        >
          <PricingPlans.PlanList.Plans
            className="mb-4 grid grid-cols-1 grid-rows-[repeat(auto-fit,minmax(0,auto))] justify-center gap-6 sm:grid-cols-[repeat(auto-fit,minmax(344px,max-content))]"
            loadingState={
              <>
                {/* Skeleton loader for the plan list */}
                <div className="aspect-[3/4] w-full animate-pulse rounded-lg bg-foreground/5"></div>
                <div className="hidden aspect-[3/4] w-full animate-pulse rounded-lg bg-foreground/5 sm:block"></div>
                <div className="hidden aspect-[3/4] w-full animate-pulse rounded-lg bg-foreground/5 lg:block"></div>
              </>
            }
          >
            <PricingPlans.PlanList.PlanRepeater>
              <article
                className={`row-span-7 grid w-full max-w-full grid-rows-subgrid gap-0 rounded-lg border border-foreground/15 text-foreground shadow-xl sm:max-w-[344px]`}
              >
                ...
                <section className="row-span-5 grid grid-rows-subgrid gap-4 px-3 py-6 text-center sm:px-6">
                  ...
                  <PricingPlans.Plan.Description
                    className="row-start-3 mx-2 text-balance font-paragraph text-foreground/80"
                    asChild
                  >
                    <p />
                  </PricingPlans.Plan.Description>
